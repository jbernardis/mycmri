...
class NodeSendThread (threading.Thread):
	...
	def send(self, omsg):
		...
		self.port.write(b'\xFF')
		self.port.write(STX)
		if bytes([ob[0]]) == IDENTIFY:
			self.port.write(bytes([48])) # broadcast addr
		else:
	...
	def recv(self):
		...
		if len(addr) == 0:
			raise NodeReadException
		# self.address == NONE  => we haven't yet learned our address.  accept anything
		if self.addr is not None and addr != bytes([65 + self.addr]):
